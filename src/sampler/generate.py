...
from utils.model_utils import create_model_and_diffusion, load_saved_model
from utils import dist_utils
from utils.sampler_utils import ClassifierFreeSampleModel, AutoRegressiveSampler
...
from data.humanml.utils import paramUtil

def main(args = None):
    # if args is None:
    #     args = generate_args()
    fixseed(args.experiment.seed)
    out_path = args.sampling.output_dir
    if args.data.dataset == 'realsense-mmb-walk':
        njoints_body = 24
    ...
    if args.data.context_len > 0:
        is_using_data = True
    dist_utils.setup_dist(args.training.device) 
    if out_path == '':
        out_path = os.path.join(os.path.dirname(args.sampling.model_path), 'samples_{}_{}_seed{}'.format(name, args.experiment.seed))

        if args.sampling.text_prompt != '':
            out_path += '_' + args.sampling.text_prompt.replace(' ', '_').replace('.','')
        elif args.sampling.input_text != '':
            out_path += '_' + os.path.basename(args.sampling.input_text).replace('.txt', '').replace(' ', '_').replace('.','')
        elif args.sampling.dynamic_text_path != '':
            out_path += '_' + os.path.basename(args.sampling.dyanmic_text_path).replace('.txt', '').replace(' ', '_').replace('.','')

    texts = None
    if args.sampling.text_prompt != '':
        texts = [args.sampling.text_prompt] * args.sampling.num_samples
    elif args.sampling.input_text != '':
        assert os.path.exists(args.sampling.input_text, 'r') 
        with open(args.sampling.input_text, 'r') as fr:
            texts = fr.readlines()
        texts = [s.replace('\n', '') for s in texts]
        args.sampling.num_samples = len(texts)
    elif args.sampling.dynamic_text_path != '':
        assert os.path.exists(args.sampling.dynamic_text_path)
        assert args.sampling.autoregressive, "Dynamic text sampling is only supported with autoregressive sampling"
        with open(args.sampling.dyanmic_text_path, 'r') as fr:
            texts = fr.readlines()
        texts = [s.replace('\n', '') for s in texts]
        n_frames = len(texts) * args.data.pred_len
    elif args.sampling.action_name:
        action_text = [args.sampling.action_name]
        args.sampling.num_samples = 1
    elif args.sampling.action_file != '':
        assert os.path.exists(args.sampling.action_file)
        with open(args.sampling.action_file, 'r') as fr:
            action_text = fr.readlines()
        action_text = [s.replace('\n', '') for s in action_text]
        args.sampling.num_samples = len(action_text)
    ...
    if args.sampling.autoregressive:
        sample_cls = AutoRegressiveSampler(args, sample_fn, n_frames)
        sample_fn = sample_cls.sample
    
    print(f"Loading checkpoints from {args.sampling.model_path}")
    load_saved_model(model, args.sampling.model_path, use_avg=args.training.use_ema)

    if args.sampling.guidance_param != 1:
        model = ClassifierFreeSampleModel(model)
    model.to(dist_utils.dev())
    model.eval()

    # keep the same representation of the motion, irrespective of the dataset. 
    motion_shape = (args.data.batch_size, model.njoints, model.nfeats, n_frames)

    if is_using_data:
        iterator = iter(data)
        input_motion, model_kwargs = next(iterator)
        input_motion = input_motion.to(dist_utils.dev())
        if texts is not None:
            model_kwargs['y']['text'] = texts
    else:
        collate_args = [{'inp': torch.zeros(n_frames), 'tokens': None, 'lengths': n_frames}] * args.sampling.num_samples
        is_t2m = any([args.sampling.input_text, args.sampling.text_prompt])
        if is_t2m:
    ...
    if args.model.cond_mode == 'no_cond':
        pass

    model_kwargs['y'] = {'key': val.to(dist_utils.dev()) if torch.is_tensor(val) else val for key, val in model_kwargs['y'].items()}
    init_image = None
    ...
    if args.sampling.guidance_param != 1:
        model_kwargs['y']['scale'] = torch.ones(args.data.batch_size, device = dist_utils.dev()) * args.sampling.guidance_param 
    
    if 'text' in model_kwargs['y'].keys():
        model_kwargs['y']['text_embed'] = model.encode_text(model_kwargs['y']['text'])

    if args.sampling.dynamic_text_path != '':
        model_kwargs['y']['text'] = [model_kwargs['y']['text']] * args.sampling.num_samples

        if args.model.text_encoder_type == 'bert':
    ...
    for rep_i in range(args.sampling.num_repetitions):
        ...
        _len = np.array([nframes for _ in range(bs)])
        if 'prefix' in model_kwargs['y'].keys():
            _len[:] = sample.shape[-1]
        all_lengths.append(_len)
        print(f"created {len(all_motions) * args.data.batch_size} samples")
    ...
    sample_file_template, row_file_template, all_file_template = construct_template_variables(args.training.unconstrained)
    max_vis_samples = 6
    num_vis_samples = min(args.sampling.num_samples, max_vis_samples)
    animations = np.empty(shape=(args.sampling.num_samples, args.sampling.num_repetitions), dtype=object)
    max_length = max(all_lengths)

    for sample_i in range(args.sampling.num_samples):
        rep_files = []
        for rep_i in range(args.sampling.num_repetitions):
            caption = all_text[rep_i*args.data.batch_size + sample_i]
            if args.sampling.dynamic_text_path != '':  # caption per frame
