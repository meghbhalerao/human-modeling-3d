...
from data.humanml.motion_loaders.model_motion_loaders import get_mdm_loader
from data.humanml.utils.metrics import *
from data.humanml.networks.evaluator_wrapper import EvaluatorMDMWrapper
from collections import OrderedDict
from data.humanml.scripts.motion_process import *
from data.humanml.utils.utils import *
...
from trainer.train_platforms import ClearmlPlatform, TensorboardPlatform, NoPlatform, WandBPlatform
...
def main(cfg: DictConfig):
    ...
    # Batch size must be 32 for R precision calculation
    batch_size = 32
    
    name = os.path.basename(os.path.dirname(cfg.evaluation.model_path))
    niter = os.path.basename(cfg.evaluation.model_path).replace('model', '').replace('.pt', '')
    log_name = 'eval_humanml_{}_{}'.format(name, niter)
    if cfg.evaluation.guidance_param != 1.:
        log_name += f'_gscale{cfg.evaluation.guidance_param}'
    log_name += f'_{cfg.evaluation.eval_mode}'
    log_file = os.path.join(os.path.dirname(cfg.evaluation.model_path), log_name + '.log')
    save_dir = os.path.dirname(log_file)

    print(f'Will save to log file [{log_file}]')

    eval_platform_type = eval(cfg.experiment.train_platform_type)
    eval_platform = eval_platform_type(save_dir, name=log_name)
    eval_platform.report_args(cfg, name='Args')
    ...
    if cfg.evaluation.eval_mode == 'debug':
        ...
        diversity_times = 300
        replication_times = 5
    elif cfg.evaluation.eval_mode == 'wo_mm':
        num_samples_limit = 1000
        run_mm = False
        mm_num_samples = 0
        mm_num_repeats = 0
    ...
    elif cfg.evaluation.eval_mode == 'mm_short':
        num_samples_limit = 1000
        run_mm = True
        mm_num_samples = 100
        mm_num_repeats = 30
        ...
        diversity_times = 300
        replication_times = 5
    elif cfg.evaluation.eval_mode == 'full':
        num_samples_limit = 1000
        run_mm = False
        mm_num_samples = 0
        mm_num_repeats = 0
        ...
        raise ValueError(f"Unknown eval_mode: {cfg.evaluation.eval_mode}")
    ...
    gen_loader = get_dataset_loader(
        ...
        split=split, 
        hml_mode='eval',
        fixed_len=cfg.data.context_len + cfg.data.pred_len, 
        pred_len=cfg.data.pred_len, 
        device=dist_utils.dev(),
        autoregressive=cfg.evaluation.autoregressive
    )

    num_actions = gen_loader.dataset.num_actions

    logger.log("Creating model and diffusion...")
    model, diffusion = create_model_and_diffusion(cfg, gen_loader)

    logger.log(f"Loading checkpoints from [{cfg.evaluation.model_path}]...")
    load_saved_model(model, cfg.evaluation.model_path, use_avg=cfg.training.use_ema)
    ...
    eval_motion_loaders = {
        'vald': lambda: get_mdm_loader(
            ...
            scale=cfg.evaluation.guidance_param
        )
    }

    eval_wrapper = EvaluatorMDMWrapper(cfg.data.dataset, dist_utils.dev())
    evaluation(
        eval_wrapper, 
        gt_loader, 
        eval_motion_loaders, 
