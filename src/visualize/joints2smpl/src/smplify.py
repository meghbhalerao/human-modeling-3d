...
class SMPLify3D():
    ...
    def __init__(self,
        ...
        # GMM pose prior
        self.pose_prior = MaxMixturePrior(prior_folder=config.GMM_MODEL_DIR, num_gaussians=8, dtype=torch.float32).to(device)
        # collision part
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        ...
        print(f"init_betas shape: {init_betas.shape}")
        print(f"init_cam_t shape: {init_cam_t.shape}")
        ...
        smpl_output = self.smpl(global_orient=global_orient, body_pose=body_pose, betas=betas, pose2rot = True)
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        if self.use_lbfgs:
            camera_optimizer = torch.optim.LBFGS(camera_opt_params, max_iter=self.num_iters, lr=self.step_size, line_search_fn='strong_wolfe')
            for i in range(10):
                def closure():
